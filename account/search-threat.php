...
if (isset($_SESSION["is_logged_in"]) && $_SESSION["is_logged_in"] && isset($_SESSION["user"]) && is_array($_SESSION["user"]) && count($_SESSION["user"]) == 8) {
    ...
    $res = json_decode($controller->getAllThreats(), true);
    $threats = $res['message'];
} else {
...
<head>
    ...
    <title>Search Threats - ArchiveIot</title>
            ...
            <section id="profile">
                <div class="search">
                    <div class="formControl">
                        <input type="search" placeholder="Enter search word" name="search_tags" id="search_tags" required minlength="6">
                    </div>
                </div>
            </section>
            <section id="table">
...
<script>
    ...
    $(document).ready(function() {
        const table = $('#post_table').DataTable({
            dom: 'ti',
