...
if (isset($_SESSION["is_logged_in"]) && $_SESSION["is_logged_in"] && isset($_SESSION["user"]) && is_array($_SESSION["user"]) && count($_SESSION["user"]) == 8) {
    ...
    if (isset($_POST["btnUpdateReport"])) {
        ...
        if (strlen(trim($threatTitle)) > 4 && strlen(trim($affectedDevices)) > 2 && strlen(trim($iocs)) > 3 && strlen(trim($description)) > 10) {
            ...
            if ($attachments["size"] > 100) {
                $response = json_decode($controller->modifyThreatReport($payload, $attachments), true);
            } else {
                $response = json_decode($controller->modifyThreatReport($payload, null), true);
            }
    ...
    $threatCount = $controller->getCount((int) $_SESSION["user"]['user_id']);
    if (isset($_GET["slug"]) && strlen(trim($_GET["slug"])) > 5) {
        $res = json_decode($controller->getThreatReportBySlug($_GET["slug"]), true);
        $threat = $res['message'];
    }
...
<head>
    ...
    <title>User Account - ArchiveIot</title>
            ...
            <section id="create_threat">
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    <div class="formControl">
                        ...
                        <input type="text" class="form-control" id="threatTitle" value="<?php echo $threat["threat_title"]; ?>" name="threatTitle" required>
                    </div>
                    ...
                    <div class="formControl">
                        ...
                        <select class="form-control" id="threatType" name="threatType" required>
                            ...
                            <option value="<?php echo $threat["threat_category"]; ?>"><?php echo ucfirst($type[$threat["threat_category"]]); ?></option>

                            <?php
                            foreach ($type as $key => $value) {
                                if ($threat["threat_category"] == $key) continue;
                            ?>
                    ...
                    <div class="formControl">
                        ...
                        <input type="date" class="form-control" value="<?php echo $threat["date_discovered"]; ?>" id="dateDetected" name="dateDetected" required>
                    </div>

                    <div class="formControl">
                        <label for="severity">Severity</label>
                        <select class="form-control" id="severity" name="severity" required>
                            <option value="<?php echo $threat["severity_level"]; ?>"><?php echo ucfirst($threat["severity_level"]); ?></option>

                            <?php
                            foreach ($severity as $key => $value) {
                                if ($threat["severity_level"] == $value) continue;
                            ?>
                    ...
                    <div class="formControl">
                        ...
                        <textarea class="form-control" id="affectedDevices" name="affectedDevices" rows="3" required><?php echo $threat["affected_devices"]; ?></textarea>
                    </div>
                    <div class="formControl">
                        <label for="iocs">Indicators of Compromise (IoCs)</label>
                        <input type="hidden" name="_slug" value="<?php echo $threat["threat_slug"] ?>">
                        <textarea class="form-control" id="iocs" name="iocs" rows="3" required><?php echo $threat["iocs"]; ?></textarea>
                    </div>
                    <div class="formControl">
                        <label for="description">Description</label>
                        <textarea class="form-control" id="description" name="description" rows="4" required><?php echo $threat["threat_desc"]; ?></textarea>
                    </div>
                    <div class="formControl">
                        <label for="mitigationSteps">Mitigation Steps</label>
                        <textarea class="form-control" id="mitigationSteps" name="mitigationSteps" rows="3"><?php echo $threat["mitigation_steps"]; ?></textarea>
                    </div>
                    <div class="formControl">
                        <label for="attachments">Attachments</label>
                        <input type="file" class="form-control-file" id="attachments" name="attachments">
                        <input type="hidden" name="old_attachment" value="<?php echo $threat["attachment"]; ?>">
                        <div class="download" style="margin-top: 4px;">
                            <a href="<?php echo $threat["attachment"]; ?>" download=""> Click to download resources!</a>
                        </div>
                    </div>

                    <div class="formControl">
                        <button type="submit" name="btnUpdateReport">Update Report</button>
                    </div>
                </form>
            </section>
