...
if (isset($_SESSION["is_logged_in"]) && $_SESSION["is_logged_in"] && isset($_SESSION["user"]) && is_array($_SESSION["user"]) && count($_SESSION["user"]) == 8) {
    ...
    $res = json_decode($controller->getUserFollowNotifications((int) $_SESSION["user"]['user_id']), true);
    $_res = json_decode($controller->getUserReadNotifications((int) $_SESSION["user"]['user_id']), true);
    $notifications = $res['message'];
    $read_notification_id = $_res['message'];
} else {
...
<head>
    ...
    <title>Notifications - ArchiveIot</title>
                    ...
                    <tbody>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td> <?php echo in_array($notification["notification_id"], $read_notification_id) ? 'Read' : 'Not Read'; ?></td>
                                    <td>
                                        <div class="controls">
                                            <?php
                                            $_url = $path . "/account/threat.php?slug=" . $notification["notification_threat_slug"];
                                            if (!in_array($notification["notification_id"], $read_notification_id)) {
                                                $_url = $path . "/account/threat.php?slug=" . $notification["notification_threat_slug"] . "&read=" . $notification["notification_id"];
                                            }

                                            ?>
                                            <a href="<?php echo $_url; ?>">View Post</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php
